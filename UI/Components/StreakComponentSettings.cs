...
{
    public partial class StreakComponentSettings : UserControl
    {
        ...
        private RegularTimeFormatter SettingsFormatter = new RegularTimeFormatter(TimeAccuracy.Hundredths);

        public StreakComponentSettings()
        {
            InitializeComponent();
            ...
            showBestCheckBox.DataBindings.Add("Checked", this, "ShowBest", false, DataSourceUpdateMode.OnPropertyChanged);
        ...
        void goalBox_ValueChanged(object sender, EventArgs e)
        {
            String text = ((TextBox)sender).Text;
            try {
                GoalTime = TimeSpanParser.Parse(text);
            } catch (Exception ex)
            {
                GoalTime = new TimeSpan();
            }
            goalBox.Text = SettingsFormatter.Format(GoalTime);
        }     
        ...
        private int CreateSettingsNode(XmlDocument document, XmlElement parent)
        {
            ...
            SettingsHelper.CreateSetting(document, parent, "GoalTime", SettingsFormatter.Format(GoalTime)) ^
            SettingsHelper.CreateSetting(document, parent, "Font1", Font1) ^
            SettingsHelper.CreateSetting(document, parent, "Font2", Font2) ^
            SettingsHelper.CreateSetting(document, parent, "ShowBest", ShowBest) ^
